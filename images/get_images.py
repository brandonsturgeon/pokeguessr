...
BASE_URL = "http://pokepalettes.com/images/sprites/poke/bw/"

# Takes the whole list of Pokemon and splits it up into chunks
# so that we can distribute the work to threads
def break_list(lst, split_to):
    a = []
    for i in xrange(0, len(lst), split_to):
        piece = lst[i:i+split_to]
        print piece
        a.append(piece)
    return a
...
# This is the function that loops through a given chunk of
# pokemon names and gets the correct images, and saves it
failed = []
def get_images(chunk):
    for name in chunk:
        # We need to get the real number of the pokemon
        i = ALL_POKEMON.index(name)+1
        ...
        try:
            image = requests.get(full_url, timeout=5).content
            with open(name+".png", "wb") as f:
                f.write(image)
        except Exception as e:
            print ""
            print e
            print "Failed on image number {}".format(i)
            failed.append(i)
        # We should probably sleep longer, this is a mean script
        time.sleep(1)

list_chunks = break_list(ALL_POKEMON, 10)
for i, chunk in enumerate(list_chunks):
    print "Creating thread #{}".format(i)
    t = threading.Thread(target=get_images, args=(chunk,))
    t.start()
